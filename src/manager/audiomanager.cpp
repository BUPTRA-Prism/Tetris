 ...
 * 即 loops = N - 1
 */
void AudioManager::PlayMusic(ResourceManager& resourceMgr, std::string_view path, unsigned int loop) {
    Mix_Music* music = resourceMgr.GetMusic(path);
    if (!music) {
        std::cerr << "Fail to load music \"" << path << "\"" << std::endl;
        return;
    }
 ...
 */
void AudioManager::PlayChunk(ResourceManager& resourceMgr, std::string_view path, unsigned int loop) {
    Mix_Chunk* chunk = resourceMgr.GetChunk(path);
    if (!chunk) {
        std::cerr << "Fail to load chunk \"" << path << "\"" << std::endl;
        return;
    }
