...
OptionIcon::OptionIcon(Context& ctx, SDL_Texture* texture, std::function<SDL_Point(int, int)> func, int m_showFrameTarget, int m_hideFrameTarget, SDL_RendererFlip flip)
    : m_ctx(ctx)
    , m_texture(texture)
    , m_func(func)
...
void OptionIcon::onRender(int row, int col) {
    SDL_Renderer* rdr = m_ctx.renderer;
    SDL_Point pos = m_func(row, col);
